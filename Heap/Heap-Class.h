...
using namespace std;
// Свойства кучи:
// У корня всегда приоритет над предком (Значение в любой узле не меньше, чем значения её потомков)
// Завершённая дерево (Последний слой заполняется слева направо без пропусков)
...
public:
    ...
    T extractMax() {
        if (heap.empty()) throw out_of_range("КУЧА ПУСТА!"); // Если куча пуская то вызывается исключение
        T max = heap[0];
        heap[0] = heap.back();
        heap.pop_back();
...
/*
Проверка исключением для пустой кучи
+ комментарии
проверка для наилучшего/худшего случая сортировки
+ комментарии (// Сортировка кучей от наименьшего к наибольшему)
сортировка будет использовать метод класса
*/